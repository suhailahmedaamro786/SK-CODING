...
</style></head><body><div class="topbar">✨ Free Express Shipping on Orders Over $50 · Use code <b>NOVAFREE</b></div><div class="shell"><header class="header"><div class="logo">${escapeHtml(projectName)}</div><nav class="nav"><a href="#">Home</a><a href="#">Shop</a><a href="#">Categories</a><a href="#">Deals</a></nav><div class="search">⌕ Search products…</div><div class="cart">♡ 2 · 🛒 3</div></header><section class="hero"><div><div class="eyebrow">New season collection</div><h1>Everything you need. Beautifully curated.</h1><p>${escapeHtml(prompt).slice(0,180)}</p><button>Shop collection →</button></div><div class="hero-art"></div></section><div class="section-head"><h2>Featured products</h2><span>12+ products · Multiple categories</span></div><section class="grid">${cards}</section><footer class="footer"><b>${escapeHtml(projectName)}</b><span>Secure checkout · Fast delivery · Easy returns</span></footer></div><script>const cart=[];const shell=document.querySelector(".shell");const original=shell.innerHTML;function page(name){const grid=document.querySelector(".grid");const head=document.querySelector(".section-head h2");if(!grid||!head)return;if(name==="cart"){head.textContent="Shopping cart";grid.innerHTML=cart.length?cart.map((x,i)=>"<article class=\"product\"><div class=\"product-body\"><div class=\"category\">CART ITEM</div><h3>"+x+"</h3><div class=\"price\">$129</div><button data-remove=\""+i+"\">Remove</button></div></article>").join(""):"<article class=\"product\"><div class=\"product-body\"><h3>Your cart is empty</h3><p>Add products from the Shop.</p></div></article>";}else if(name==="shop"||name==="categories"){head.textContent=name==="shop"?"All products":"Categories";}else{head.textContent="Featured products";}}function route(){const n=location.hash.replace("#","")||"home";page(n);document.querySelectorAll("[data-route]").forEach(a=>a.addEventListener("click",()=>setTimeout(route,0)));}document.addEventListener("click",e=>{const b=e.target.closest("button");if(!b)return;if(b.textContent.includes("Add to cart")){const title=b.closest(".product")?.querySelector("h3")?.textContent;if(title){cart.push(title);b.textContent="Added ✓";setTimeout(()=>b.textContent="Add to cart",900);}}if(b.dataset.remove){cart.splice(Number(b.dataset.remove),1);page("cart");}});document.querySelectorAll("nav a").forEach((a,i)=>{a.dataset.route="1";if(i===0)a.href="#home";if(i===1)a.href="#shop";if(i===2)a.href="#categories";if(i===3)a.href="#shop";});document.querySelector(".cart")?.addEventListener("click",()=>{location.hash="#cart";route();});window.addEventListener("hashchange",route);route();</script></body></html>`;
}
