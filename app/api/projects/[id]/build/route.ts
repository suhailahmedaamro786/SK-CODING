  ...
  try {
    ...
    if (!plan) {
      ...
      await supabase.from("projects").update({
        status: "planning",
        specification: { summary: parsed.summary ?? "", initial_prompt: prompt },
    ...
    await supabase.from("build_logs").insert({ project_id: id, clerk_user_id: user.id, level: "info", message: "Build started from user prompt." });
...
Generate a complete coherent Next.js application. Return ONLY {"files":[{"path":"...","content":"..."}]}.
Required: package.json, tsconfig.json, next-env.d.ts, app/layout.tsx, app/globals.css, app/page.tsx. Include the requested UI and core features. Use Next.js App Router, TypeScript and Tailwind. It must run with npm install && npm run build. No markdown fences.` }
      ],
...
Generate additional feature files, components, pages, types, API routes where useful and README. Do not repeat package.json, tsconfig.json, next-env.d.ts, app/layout.tsx, app/globals.css, app/page.tsx. Keep it compile-ready.` }
    ...
    const merged = [...safeFiles(core.files), ...safeFiles(features.files)]
      .filter((file, index, arr) => arr.findIndex(x => x.path === file.path) === index)
      .slice(0, 32);
    ...
    if (tasks.length) await supabase.from("project_tasks").upsert(tasks.slice(0, 30).map((t: any, i: number) => ({
      ...
      files: t.files || [], acceptance_criteria: t.acceptance_criteria || [],
    ...
    await supabase.from("build_logs").insert({ project_id: id, clerk_user_id: user.id, level: "info", message: `Build completed: ${merged.length} files generated.` });
