...
export default function ProjectWorkspace({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className={(dark ? "bg-[#07080c] text-zinc-100" : "bg-slate-100 text-slate-900") + " min-h-screen transition-colors duration-300"}>
      <header className={(dark ? "border-white/10 bg-[#08090d]/90" : "border-slate-200 bg-white/90") + " sticky top-0 z-40 flex h-16 items-center gap-3 border-b px-4 backdrop-blur-xl sm:px-6"}>
        ...
        <span className="max-w-[180px] truncate text-sm font-semibold">{project?.name || "Workspace"}</span>
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <UserButton />
          <button onClick={deploy} disabled={deploying || !files.length} className="hidden items-center gap-2 rounded-xl bg-violet-500 px-3.5 py-2 text-xs font-bold text-white shadow-lg shadow-violet-500/20 transition hover:-translate-y-0.5 hover:bg-violet-400 disabled:cursor-not-allowed disabled:opacity-40 sm:inline-flex">
      ...
      <div className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-[1900px] lg:grid-cols-[minmax(360px,430px)_minmax(0,1fr)]">
        <aside className={(dark ? "border-white/10 bg-[#090a0e]" : "border-slate-200 bg-white") + " order-2 flex min-h-[680px] flex-col border-b lg:order-1 lg:border-b-0 lg:border-r"}>
          ...
          <div className={(dark ? "border-white/10 bg-[#090a0e]" : "border-slate-200 bg-white") + " border-t p-4"}>
            <textarea
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendPrompt(); } }}
              rows={5}
              placeholder={files.length ? "Tell SK Builder what to change…" : "Describe the website you want to build…"}
              className={(dark ? "border-white/10 bg-white/[.035] text-zinc-100 placeholder:text-zinc-600" : "border-slate-200 bg-slate-50 text-slate-900 placeholder:text-slate-400") + " w-full resize-none rounded-2xl border p-3.5 text-sm outline-none transition focus:border-violet-400 focus:ring-2 focus:ring-violet-500/10"}
            />
            ...
            <p className="mt-2 text-center text-[10px] text-zinc-600">Enter to send · Shift+Enter for a new line</p>
          </div>
        </aside>

        <section className={(dark ? "bg-[#101217]" : "bg-slate-50") + " order-1 min-w-0 lg:order-2"}>
          ...
          <div className="relative h-[calc(100vh-8rem)] min-h-[620px] p-3 sm:p-5">
            ...
            <div className="mx-auto mt-3 flex max-w-[900px] items-center justify-center gap-2">
              ...
              <span className="hidden text-[10px] text-zinc-600 sm:inline">Click Files or Code — no permanent folder sidebar.</span>
            </div>
          </div>
        </section>
      </div>

      <footer className={(dark ? "border-white/10 bg-[#07080c]" : "border-slate-200 bg-white") + " border-t py-5 text-center text-[11px] text-zinc-500"}>
        Powered by <span className="font-semibold">SUHAIL AHMED AAMRO</span> · <a href="https://my-portfilo-41201.vercel.app" target="_blank" rel="noreferrer" className="hover:text-violet-400">Portfolio</a>
      </footer>
    </main>
  );
}
